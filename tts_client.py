...
from tts_ext import EVENT_TTSResponse, EVENT_TTSSentenceStart, EVENT_TTSSentenceEnd, EVENT_SessionStarted, \
    EVENT_SessionFinished, EVENT_ConnectionStarted, EVENT_ConnectionFailed, EVENT_ConnectionFinished, print_log
...
async def send_messages(ws):
    ...
    for text_segment in text_segments:
        ...
        print_log(f'text_segment -> {text_segment}')
...
async def receive_messages(ws: websockets):
    ...
    try:
        ...
        async for message in ws:
            print_log(f"收到消息: {message[:100]}...")  # 只打印消息的前100个字符
            ...
            if event == EVENT_SessionStarted:
                ...
                print_log(f"音频将保存到: {output_file}")
                # 清空或新建文件
                ...
                print_log(f'未处理的事件: {event} {data}')

    except websockets.exceptions.ConnectionClosed:
        print_log("连接已关闭，停止接收。")
    except Exception as e:
        print_log(f"接收消息时发生错误: {str(e)}")
    finally:
        print_log(f"音频已保存到: {output_file}")
